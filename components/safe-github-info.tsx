...
import { fetchRepositoryInfo, type FetchRepositoryInfoOptions } from './github-info';
...
type SafeGithubInfoProps = ComponentProps<'a'> & FetchRepositoryInfoOptions;
    ...
    >
      ...
      {info ? (
        <div className="flex text-xs items-center gap-1 text-fd-muted-foreground">
          ...
          <span>{compactFormatter.format(info.stars)}</span>
          <GitFork className="size-3 ms-2" />
          <span>{compactFormatter.format(info.forks)}</span>
        </div>
      ) : (
        <div className="flex text-xs items-center gap-1 text-fd-muted-foreground">
